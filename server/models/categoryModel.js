...
export const getCategory3 = (result) => {
  ...
  export const getSubCategories = (id, result) => {
    db.query("SELECT *, (SELECT GROUP_CONCAT(category3Name) FROM category3 WHERE category2ID = category2.category2ID) AS category3Names FROM category2 WHERE category1ID = ?", [id], (err, results) => {
      if (err) {
        console.log(err)
        result(err, null)
      } else {
